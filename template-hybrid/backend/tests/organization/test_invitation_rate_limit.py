...
/ #31 / RBAC #18): without it, a compromised or careless account could
blast invitations at arbitrary emails all day."""
...
async def test_invitations_are_rate_limited_per_org(
    ...
    third = await client.post(
        f"/v1/organizations/{org_a}/invitations",
        json={"email": "invitee-2@example.com", "role": "member"},
        headers=auth_header(token_alice),
    )
...
async def test_invitations_are_rate_limited_per_inviter(
    ...
    across many orgs it belongs to -- this is the per-user half of the
    same guard."""
    ...
    first = await client.post(
        f"/v1/organizations/{org_a}/invitations",
        json={"email": "invitee-0@example.com", "role": "member"},
        headers=auth_header(token_alice),
    )
    ...
    second = await client.post(
        f"/v1/organizations/{org_a}/invitations",
        json={"email": "invitee-1@example.com", "role": "member"},
        headers=auth_header(token_alice),
    )
...
async def test_an_org_exceeded_request_does_not_burn_the_inviters_own_quota(
    ...
    invites they send from a different org they belong to).
    ...
    second org she also belongs to as owner, org_b.
    """
    ...
    org_b = await create_organization(session, name="Second Org")
    await add_member(
        session, organization_id=org_b.id, user_id=alice_id, role=MemberRole.owner
    )

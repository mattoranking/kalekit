...
async def add_member(
    ...
    """add_member(organization_id, invitee_token, invitee_email, role=...) -> None.

    Gets `invitee_email` into `organization_id` at `role` via the real
    invitation flow: creates the invitation row directly (bypassing the
    invite endpoint's email delivery, which tests have no way to
    intercept since only the token's hash is ever persisted -- mirrors
    how RBAC's tests construct `EmailVerificationToken` rows directly)
    and accepts it through the actual `/v1/invitations/accept`
    endpoint, using the organization's own owner as the recorded
    inviter -- an owner can grant any role, so this fixture never trips
    the accept-time "inviter can still grant this role" check.
    """

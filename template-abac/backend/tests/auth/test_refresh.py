...
async def test_concurrent_refresh_within_grace_window_returns_same_pair(
    ...
    (e.g. two BFF instances) must both succeed and get back the
    identical new pair, not a fresh pair each -- otherwise one of two
    legitimate concurrent refreshes ends up holding a token the other
    invalidated.
    """
...
@pytest.mark.asyncio(loop_scope="session")
async def test_two_sessions_are_independent(client: AsyncClient) -> None:
    """Refreshing one session (family) must not revoke or otherwise
    disturb a second, unrelated session for the same user -- the
    exact bug #4 describes: /auth/refresh used to call
    revoke_user_refresh_tokens(user.id), killing every other
    ...
    rotate_a = await _refresh(client, refresh_token_a)
    assert rotate_a.status_code == 200

    # Session B's refresh token must still work -- refreshing A didn't
    # touch B's family.
    rotate_b = await _refresh(client, refresh_token_b)
...
async def test_refresh_for_deactivated_user_returns_401(
    ...
    _, refresh_token = await _login_pair(client, "deactivated@example.com")

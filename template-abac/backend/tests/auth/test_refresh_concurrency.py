...
template-hybrid's `test_role_change_concurrency.py` for the same
argument in a different race. Instead, `get_refresh_token_by_hash_for_
update` (as imported into `kalekit.auth.endpoints`, which is what
`refresh()` actually calls) is monkeypatched so that, once the first
request's `SELECT ... FOR UPDATE` has acquired the row lock, a second,
fully independent request for the *same* token is started concurrently
and asserted to still be blocked a moment later -- deterministically
proving the lock is what's serializing them, not scheduler luck.
"""

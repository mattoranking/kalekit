...
`refresh()` revokes a token family and then raises HTTPException(401).
...
@pytest.mark.asyncio(loop_scope="session")
async def test_absolute_timeout_revoke_persists_through_real_session(
    real_client: AsyncClient, engine: AsyncEngine, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    try:
        token = await _login(real_client, email)
        r = await real_client.post("/v1/auth/refresh", json={"refresh_token": token})
        assert r.status_code == 401
        assert r.json() == {"detail": "Session expired"}

        flags = await _family_revoked_flags(engine, email)
        assert flags and all(flags), "family revoke was rolled back"
    finally:
        await _cleanup(engine, email)
